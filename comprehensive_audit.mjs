// Comprehensive QMS Audit Script - ES Module version
...
// Define required fields per form based on the schema
const SCHEMAS = {
  ...
  "F/29": { name: "Training Record", required: ["serial", "employee_name", "employee_id", "department", "course_name", "training_date", "trainer", "recorded_by"], tableFields: [] },
...
async function runAudit() {
  ...
  const findings = [];
  const counters = { emptyFields: 0, invalidValues: 0, inconsistencies: 0, duplicates: 0 };
  ...
  for (const [formCode, formRecords] of Object.entries(byForm)) {
    ...
    for (const record of formRecords) {
      ...
      validateFormSpecific(formCode, schema, record, formData, findings, counters);
    }
  }
  
  // Also check risks and capas tables
  console.log('\n=== Auditing risks table ===');
  await auditRisks(findings, counters);
  
  console.log('\n=== Auditing capas table ===');
  await auditCapas(findings, counters);
  
  // Generate report
  generateReport(findings, records.length, Object.keys(byForm).length, counters);
}

function validateFormSpecific(formCode, schema, record, formData, findings, counters) {
...
async function auditRisks(findings, counters) {
...
async function auditCapas(findings, counters) {
...
function generateReport(findings, totalRecords, totalForms, counters) {
