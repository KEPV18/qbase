...
const AUTH_LOCAL_DISABLED = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_AUTH_LOCAL_DISABLED === "true") || true;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchFullUsersList = React.useCallback(async () => {
    ...
    try {
      ...
      console.error("[useAuth:fetchFullUsersList] Failed to load users:", (e as Error).message);
    }
  }, [supabaseDisabled]);
  ...
  React.useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      try {
        ...
        console.error("[useAuth:bootstrap] Session initialization failed:", (err as Error).message);
        if (mounted) setLoading(false);
      }
      bootstrapDone = true;
    };
    ...
    const run = async () => {
      if (supabase && !supabaseDisabled) {
        try {
          ...
          console.error("[useAuth:reloadUsers] Failed to reload user list:", (e as Error).message);
          setSupabaseDisabled(true);
        }
      } else {
        if (!AUTH_LOCAL_DISABLED) {
    ...
    if (backend === "supabase") {
      try {
        ...
        try {
          ...
          console.error("[useAuth:login] Password fallback failed:", (fallbackErr as Error).message);
        }
        console.error("[useAuth:login] Supabase authentication failed");
      }
    }
  ...
  const addUser = React.useCallback(async (userInput: Omit<AppUser, "id">) => {
    ...
    if (supabase) {
      ...
      if (profileErr) {
        console.error("[useAuth:addUser] Profile insert failed:", profileErr.message);
      } else {
      ...
      try {
        ...
        if (roleErr) {
          console.error("[useAuth:addUser] Role insert failed:", roleErr.message);
        } else {
          // Role assigned successfully
        }
      } catch (e: unknown) {
        console.error("[useAuth:addUser] Unexpected error:", (e as Error).message);
      }
  ...
  const updateUser = React.useCallback(async (id: string, updates: Partial<AppUser>) => {
    ...
    if (supabase) {
      ...
      if (Object.keys(payload).length > 0) {
        ...
        if (error) {
          console.error("[useAuth:updateUser] Profile update by user_id failed:", error.message);
          const { error: err2 } = await supabase.from("profiles").update(payload).eq("id", id);
          if (err2) {
            console.error("[useAuth:updateUser] Profile update by id also failed:", err2.message);
            failed = true;
          }
        }
      }
      ...
      if (typeof updates.role === "string" && !failed) {
        ...
        try {
          ...
          if (!validRoles.includes(roleToSave)) {
            console.error("[useAuth:updateUser] Invalid role:", roleToSave);
            failed = true;
          } else {
            ...
            if (roleErr) {
              ...
              if (insErr) {
                console.error("[useAuth:updateUser] Role insert fallback failed:", insErr.message);
                failed = true;
              }
            }
          }
        } catch (e: unknown) {
          console.error("[useAuth:updateUser] Role update failed:", (e as Error).message);
          failed = true;
        }
      }
  ...
  const removeUser = React.useCallback(async (id: string) => {
    ...
    if (supabase) {
      ...
      if (roleErr) console.error("[useAuth:removeUser] Role delete failed:", roleErr.message);
      const { error: profErr } = await supabase.from("profiles").delete().eq("user_id", id);
      if (profErr) {
        console.error("[useAuth:removeUser] Profile delete by user_id failed:", profErr.message);
        // Fallback to id column
        const { error: profErr2 } = await supabase.from("profiles").delete().eq("id", id);
        if (profErr2) console.error("[useAuth:removeUser] Profile delete by id also failed:", profErr2.message);
      }
      await reloadUsers();
    }
  }, [users, user, reloadUsers]);
  ...
  const resetPassword = React.useCallback(async (email: string): Promise<{ ok: boolean; message: string }> => {
    ...
    try {
      ...
      if (error) {
        console.error("[useAuth:resetPassword] Reset request failed:", error.message);
        return { ok: false, message: error.message };
      }
      return { ok: true, message: `Password reset link sent to ${email}` };
    } catch (e: unknown) {
      console.error("[useAuth:resetPassword] Unexpected error:", (e as Error).message);
