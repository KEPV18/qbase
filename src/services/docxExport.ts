      ...
      new TableRow({
        children: [
          new TableCell({
            ...
            children: [new Paragraph({ children: [new TextRun({ text: String(label), bold: true, size: 20, font: 'Outfit', color: SLATE_700 })] })],
          }),
          new TableCell({
...
function buildFieldsTable(schema: FormSchema, record: Record<string, unknown>): (Paragraph | Table)[] {
...
function buildSubTables(schema: FormSchema, record: Record<string, unknown>): (Paragraph | Table)[] {
  const paragraphs: (Paragraph | Table)[] = [];
...
function buildFooter(record: Record<string, unknown>): Paragraph[] {
  ...
  return [
    ...
    new Paragraph({
      spacing: { before: 100 },
      border: { top: { style: BorderStyle.SINGLE, size: 1, color: SLATE_300 } },
      children: [
        new TextRun({ text: `Generated by QBase — ${serial} — Exported ${exported}`, size: 16, color: SLATE_500, font: 'Outfit', italics: true }),
      ],
    }),
  ];
}
...
export async function exportRecordsToDocx(records: Record<string, unknown>[]): Promise<void> {
  ...
  const sections: { properties: { page: { margin: { top: number; right: number; bottom: number; left: number } } }; children: (Paragraph | Table)[] }[] = [];
  ...
  const filename = `QBase_Export_${records.length}_records_${timestamp}.docx`;
  saveAs(blob, filename);
}